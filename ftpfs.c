...
static void cancel_previous_multi()
{
	...
	CURLMcode curlMCode = curl_multi_remove_handle(ftpfs.multi,
						ftpfs.connection);
			       ...
			       int update_offset)
{
	...
	if ((fh->buf.len < size + offset - fh->buf.begin_offset) ||
		...
		if (ftpfs.current_fh != fh ||
			...
			curl_easy_setopt_or_die(ftpfs.connection,
						CURLOPT_URL, full_path);
			curl_easy_setopt_or_die(ftpfs.connection,
						CURLOPT_WRITEDATA, &fh->buf);
			if (offset) {
				char range[15];
			...
			CURLMcode curlMCode =  curl_multi_add_handle(ftpfs.multi,
								ftpfs.connection);
			if (curlMCode != CURLE_OK) {
				fprintf(stderr,
					"curl_multi_add_handle problem: %d\n",
					curlMCode);
				exit(1);
			}
		...
		while ((fh->buf.len < size + offset - fh->buf.begin_offset) &&
			...
			curl_multi_fdset(ftpfs.multi, &fdread, &fdwrite,
					&fdexcep, &maxfd);
		...
		if (running_handles == 0) {
			int msgs_left = 1;
			while (msgs_left) {
				CURLMsg* msg = curl_multi_info_read(ftpfs.multi,
								&msgs_left);
				if (msg == NULL ||
...
static void *ftpfs_write_thread(void *data)
{
	...
	curl_easy_setopt_or_die(fh->write_conn, CURLOPT_ERRORBUFFER,
				fh->curl_error_buffer);
	...
	if (fh->pos > 0) {
		...
		//curl_easy_setopt_or_die(fh->write_conn, CURLOPT_RESUME_FROM_LARGE,
		//			(curl_off_t)fh->pos);
	}
	...
	if (curl_res != CURLE_OK) {
		DEBUG(1, "write problem: %d(%s) text=%s\n", curl_res,
			curl_easy_strerror(curl_res), fh->curl_error_buffer);
			     ...
			     struct fuse_file_info* fi)
{
	...
	if ((fi->flags & O_ACCMODE) == O_RDONLY) {
		if (fi->flags & O_CREAT)
			...
			// If it's read-only, we can load the file a bit at a
			// time, as necessary.
			DEBUG(1, "opening %s O_RDONLY\n", path);
			fh->can_shrink = 1;
			size_t size = ftpfs_read_chunk(fh->full_path, NULL, 1,
						0, fi, 0);
		...
		if (!err) {
			if ((fi->flags & O_CREAT) || (fi->flags & O_TRUNC)) {
				DEBUG(1, "opening %s for writing with O_CREAT or "
					"O_TRUNC. write thread will start now\n",
					path);
				...
				DEBUG(1, "opening %s for writing without O_CREAT "
					"or O_TRUNC. write thread will start after "
					"ftruncate\n", path);
				/* expecting ftruncate */
				fh->write_may_start = 0;
			}
		      ...
		      struct fuse_file_info* fi)
{
	...
	if (fh->pos > 0 || fh->write_conn != NULL) {
		fprintf(stderr, "in read/write mode we cannot read from a file "
...
static int ftpfs_chmod(const char* path, mode_t mode)
{
	...
	curl_easy_setopt_or_die(ftpfs.connection, CURLOPT_URL, full_path);
	curl_easy_setopt_or_die(ftpfs.connection, CURLOPT_WRITEDATA, &buf);
	curl_easy_setopt_or_die(ftpfs.connection, CURLOPT_NOBODY,
				ftpfs.safe_nobody);
	CURLcode curl_res = curl_easy_perform(ftpfs.connection);
...
static int ftpfs_truncate(const char* path, off_t offset)
{
	...
	/* we can't use ftpfs_mknod here, because we don't know the right */
	/* permissions */
	if (offset == 0)
		return op_return(create_empty_file(path), "ftpfs_truncate");
	...
	DEBUG(1, "ftpfs_truncate problem: %s offset != 0 or filesize=%lld != "
...
static int ftpfs_ftruncate(const char * path, off_t offset,
			struct fuse_file_info * fi)
{
	...
	if (offset == 0) {
		if (fh->pos == 0) {
			fh->write_may_start = 1;
			return op_return(create_empty_file(fh->open_path),
					"ftpfs_ftruncate");
		}
	...
	DEBUG(1, "ftpfs_ftruncate problem: %s offset != 0 or filesize(=%lld) "
		"!= offset(=%lld)\n", path, (long long int)size,
		       ...
		       off_t offset, struct fuse_file_info *fi)
{
	...
	DEBUG(1, "ftpfs_write: %s size=%zu offset=%lld has_write_conn=%d "
		"pos=%lld\n", path, size, (long long)offset,
		fh->write_conn != 0, fh->pos);
	...
	if (!fh->write_conn && fh->pos == 0 && offset == 0) {
		DEBUG(1, "ftpfs_write: starting a streaming write at "
			"pos=%lld\n", fh->pos);

		/* check if the file has been truncated to zero or has been */
		/* newly created */
		if (!fh->write_may_start) {
			long long size = (long long int)test_size(path);
			if (size != 0) {
				fprintf(stderr, "ftpfs_write: start writing with "
					"no previous truncate not allowed! size "
					"check rval=%lld\n", size);
	...
	if (fh->write_conn) {
		...
		if (offset != fh->pos) {
			...
			sem_post(&fh->data_avail);
			/* wait until libcurl has completely written the current */
			/* chunk or finished/failed */
			sem_wait(&fh->data_written);
			fh->written_flag = 0;

			if (fh->write_fail_cause != CURLE_OK) {
				/* TODO: on error we should problably unlink the */
				/* target file  */
...
static int ftpfs_flush(const char *path, struct fuse_file_info *fi)
{
	...
	if (fh->write_conn) {
		...
		if (sbuf.st_size != fh->pos) {
			...
			fprintf(stderr, "ftpfs_flush: check filesize problem: "
				"size=%lld expected=%lld\n", sbuf.st_size,
				fh->pos);
...
static void set_common_curl_stuff(CURL* easy)
{
	...
	if (ftpfs.custom_list)
		curl_easy_setopt_or_die(easy, CURLOPT_CUSTOMREQUEST,
					ftpfs.custom_list);

	if (ftpfs.tryutf8) {
		// We'll let the slist leak, as it will still be accessible
		// within libcurl. If we ever want to add more commands to
		// CURLOPT_QUOTE, we'll have to think of a better strategy.
		struct curl_slist *slist = NULL;

		// Adding the QUOTE here will make this command be sent with
		// every request. This is necessary to ensure that the server
		// is still in UTF8 mode after we get disconnected and
	...
	curl_easy_setopt_or_die(easy, CURLOPT_CONNECTTIMEOUT,
				ftpfs.connect_timeout);
